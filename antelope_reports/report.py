...
from .charts import scenario_compare_figure, save_plot
...
def fragment_header(frag):
    s = '{\\hypertarget{%.5s}{\\Large \\texttt{%s}}}\\subsection{%s}\n' % (
        frag.uuid, frag_link_label(frag.external_ref), frag['Name']
    )
...
def subfrag_box(uid, tgt_top):
    return ('\\rput(px%.5s){\\rput(1.25,0){\\nodebox{nx%.5s}{style=process,style=dashed}'
            '{1.15cm}{0.6cm}{\\hyperlink{%.5s}{}}}}'
            '\\rput[b]([angle=90,nodesep=3pt]nx%.5s){\\texttt{%s}}'% (uid, uid, tgt_top.uuid, uid,
...
def context_box(uid):
    return '\\rput(px%.5s){\\rput(1,0){\\nodebox{nx%.5s}{style=phantom}{1em}{1em}{$\odot$}}}' % (uid, uid)
...
class TeXAuthor(object):
    ...
    def frag_layout_recurse(self, fragment, level=0, depth=0, scenario=None):
        ...
        :return:
xs        """
        coords = [('%.5s' % fragment.uuid, depth, level)]
        ...
        if len(children) > 0 and not term.is_null:
            ...
            for c in sorted(children, key=lambda x: (x['StageName'],
                ...
                #if c.exchange_value(scenario, observed=True) == 0 and not c.is_balance:
                #    continue
    ...
    def frag_layout_traverse(self, fragment, node_weight, scenario=None, first=False, parbox_width=15):
        ...
        #if node_weight == 0 and not fragment.is_balance:
        #    return '\n', '\n', subfrags
        if term.is_frag:
            if term.is_fg:
                # foreground
                boxes = fg_box(fragment.uuid)
                frag_name = fragment['Name']
            elif term.term_is_bg:
                if term.term_node.term.is_null:
                    # cutoff
                    boxes = cutoff_box(fragment.uuid)
                else:
                    # background
                    boxes = bg_box(fragment.uuid)
                frag_name = term.term_node['Name']
            else:
                # subfragment
                top_frag = term.term_node.top()
                subfrags.append(top_frag)
                boxes = subfrag_box(fragment.uuid, top_frag)
            ...
            # I/O
            boxes = io_box(fragment.uuid)
            frag_name = '%s: %s' % (fragment.direction, fragment.flow['Name'])
        elif term.is_context:
            boxes = context_box(fragment.uuid)
            frag_name = term.term_node.name

        else:
            # process
            boxes = process_box(fragment.uuid)
            frag_name = '[%s] %s' % (term.term_node.origin, term.term_node.name)
        ...
        if fragment.direction == 'Input':
            arrows = '\\ncline{->}{nx%.5s}{px%.5s}' % (fragment.uuid, fragment.uuid)
        else:
            arrows = '\\ncline{<-}{nx%.5s}{px%.5s}' % (fragment.uuid, fragment.uuid)
        if not first:
        ...
        if 1:  # node_weight != 0 or scenario is None:
            if len(children) > 0 and not term.is_null and not term.is_context:
                parbox_width -= 2.4

                for c in children:
                    arrows += '\n\\ncangle[angleA=-90,angleB=180,framearc=0]{nx%.5s}{px%.5s}' % (fragment.uuid,
                                                                                                 c.uuid)
                    arrows += '\n'
                    boxes += '\n'
    ...
    def _contrib_chart(self, f_base, results, stages=None, **kwargs):
        """
        Creates a chart and saves it in the image directory. The file name is frag.uuid (the only use of
...
''' % title
    ...
    ''' # we don't have those kinds of queries anymore
    ...
    def run_frag(self, f, scenario, quantities, **kwargs):
        res = [f.fragment_lcia(k, scenario=scenario).aggregate() for k in quantities]
        self._seen.add(f)
        children = self.fragment_report(f, scenario=scenario, results=res, **kwargs)
        new_children = [ch for ch in children if ch not in self._seen]
        return new_children
    ...
    def recurse_report(self, *fragments, quantities=None, section_names=None, scenario=None,
        ...
        while len(fragments) > 0:
            ...
            for f in fragments:
                org = f.origin
                new_children = self.run_frag(f, scenario, quantities, **kwargs)
                if single_origin:
                    new_children = [ch for ch in new_children if ch.origin == org]
                next_children.extend(new_children)
            fragments = next_children
